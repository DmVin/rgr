...
TOKEN = "6757163161:AAHEOGEmYXR4GrulHUYiKhRmgguKwBKOFeo"
...
keyboard.add(BPMN_button, about_bot_button, find_item_button, dashboard_button, order_button)


@bot.message_handler(commands=['start'])
def send_welcome(message):
    bot.send_message(message.chat.id, 'Здравствуйте. Этот бот для регистрации внутренней логистики.'
                                      'Тут можно создавать запросы на заказ, регистрировать пользователя, генерировать смотреть отчетность. ')
    bot.send_message(message.chat.id, 'Выбрать действие:', reply_markup=keyboard)

# Ответ на запрос о создателе
def About_(message):
    bot.reply_to(message, 'Д.Ю. Шестаков 2ИБ-1')
...
def Dashboard_but(message):
    ...
    bot.send_message(message.chat.id, 'Выбрать действие:', reply_markup=dashboard_markup)
...
def callback_handler(call):
    if call.data in CALLBACK_D_BUTTON:
        ...
        bot.send_message(call.message.chat.id,
                         'Telegram-бот может значительно упростить и ускорить процесс обработки заказов '
